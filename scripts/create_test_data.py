...
def create_test_data_with_inconsistencies(source_level_dir, output_dir):
    ...
    modified_description = description_data.copy()
    modified_passport = passport_data.copy()
    ...
    has_inheritance_in_desc = "inheritance" in description_data["Wealth Summary"].lower()
    has_inheritance_in_profile = "Inheritance" in profile_data.get("wealth_info", {}).get("wealth_sources", [])
    
    if has_inheritance_in_desc and has_inheritance_in_profile:
        # Remove inheritance from description
        inconsistent_description["Wealth Summary"] = inconsistent_description["Wealth Summary"].replace("inheritance", "savings")
        inconsistent_description["Wealth Summary"] = inconsistent_description["Wealth Summary"].replace("inherited", "saved")
        ...
        inconsistent_description["Wealth Summary"] += " She also inherited a significant sum from her grandparent in 2020."
    ...
    if inconsistent_profile.get("employment") and inconsistent_profile["employment"]:
        current_status = inconsistent_profile["employment"][0].get("current_status", {}).get("status_type", "")
    
    # Modify employment status in either description or profile
    if current_status == "Employee":
        # Change profile to retired
        if inconsistent_profile.get("employment") and inconsistent_profile["employment"]:
            inconsistent_profile["employment"][0]["current_status"]["status_type"] = "Retired"
    elif current_status == "Retired":
        # Change profile to employee
        if inconsistent_profile.get("employment") and inconsistent_profile["employment"]:
            inconsistent_profile["employment"][0]["current_status"]["status_type"] = "Employee"
        ...
        inconsistent_description["Occupation History"] = inconsistent_description["Occupation History"].replace(
            "not employed", "currently employed as a Marketing Director at XYZ Company"
        ).replace(
            "did not start", "started"
        )
    else:
        # Add contradictory employment info to description
        inconsistent_description["Occupation History"] += " They recently retired from their position."
    ...
    education_history = inconsistent_profile.get("personal_info", {}).get("education_history", "")
    ...
    if education_history:
        ...
        if len(education_parts) >= 2:
            ...
            inconsistent_description["Education Background"] = inconsistent_description["Education Background"].replace(
                school_name, "Different University"
            ).replace(
                graduation_year, str(int(graduation_year) + 5)
            )
    else:
        ...
        inconsistent_profile["personal_info"]["education_history"] = "University of Inconsistency (2010)"
