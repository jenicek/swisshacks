...
def decode_and_save(encoded_data, filename_prefix, output_dir='decoded_files', force_extension=None):
    ...
    try:
        ...
        if isinstance(encoded_data, str):
            ...
            try:
                ...
                print(f"Failed to decode {filename_prefix} as base64, treating as raw text")
        ...
        file_ext = force_extension if force_extension else detect_file_type(decoded_data)
        ...
        for field_name, field_value in client_data.items():
            if field_value:
                ...
                if field_name == 'profile':
                    ...
                    output_path = decode_and_save(field_value, field_name, str(output_dir), force_extension='.docx')
                else:
                    output_path = decode_and_save(field_value, field_name, str(output_dir))
