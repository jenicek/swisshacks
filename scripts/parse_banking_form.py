...
def parse_banking_form(pdf_path):
    """
...
def save_form_data_as_json(form_data, output_path=None):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parse banking forms from PDF and save as JSON')
    parser.add_argument('pdf_path', help='Path to the PDF form file')
    parser.add_argument('-o', '--output', help='Output JSON file path (default: form_data.json)')
    ...
    try:
        ...
        print(f"Total fields extracted: {len(form_data) - ('_signature_fields' in form_data)}")
